...
function collectGridSizingProps(
	node: SceneNode,
...
function collectGridPositionProps(
	node: SceneNode,
	props: [string, unknown][],
): void {
...
function collectFlexAlignmentProps(
	node: SceneNode,
	props: [string, unknown][],
): void {
	...
	else if (node.primaryAxisAlign === "SPACE_BETWEEN")
		props.push(["justify", "between"]);

	if (node.counterAxisAlign === "CENTER") props.push(["items", "center"]);
	else if (node.counterAxisAlign === "MAX") props.push(["items", "end"]);
	else if (node.counterAxisAlign === "STRETCH")
		props.push(["items", "stretch"]);
}

function collectAutoLayoutPaddingProps(
	node: SceneNode,
	props: [string, unknown][],
): void {
...
function collectCornerRadiiProps(
	node: SceneNode,
	props: [string, unknown][],
): void {
...
function collectAppearanceProps(
	node: SceneNode,
	props: [string, unknown][],
): void {
	...
	if (node.rotation !== 0)
		props.push(["rotate", Math.round(node.rotation * 100) / 100]);
...
function collectTextNodeProps(
	...
	if (node.letterSpacing !== 0)
		props.push(["letterSpacing", node.letterSpacing]);
	if (node.textDecoration !== "NONE")
		props.push(["textDecoration", node.textDecoration.toLowerCase()]);
	if (node.textCase !== "ORIGINAL")
		props.push(["textCase", node.textCase.toLowerCase()]);
	if (node.maxLines != null) props.push(["maxLines", node.maxLines]);
	if (node.textTruncation === "ENDING" && node.maxLines == null)
		props.push(["truncate", true]);
...
function collectShapeNodeProps(
	node: SceneNode,
	props: [string, unknown][],
): void {
	if (node.type === "STAR") {
		if (node.pointCount !== 5) props.push(["points", node.pointCount]);
		if (node.starInnerRadius !== 0.382)
			props.push(["innerRadius", node.starInnerRadius]);
	}
...
function nodeToJSX(
	...
	if (format === "tailwind") {
		...
		const nameAttr =
			node.name && node.name !== node.type ? ` data-name="${node.name}"` : "";
		const classAttr =
